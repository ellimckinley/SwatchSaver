document.addEventListener("DOMContentLoaded", function () {
    // Load sections based on localStorage or initialize with 5 sections
    loadSections();
...
function loadSections() {
    ...
    if (sections.length === 0) {
        // If no sections are saved, initialize 5 sections
        for (let i = 0; i < 5; i++) {
            addNewSection();
        }
    } else {
        // Otherwise, recreate sections from localStorage
        sections.forEach((color) => addNewSection(color));
    }
}
